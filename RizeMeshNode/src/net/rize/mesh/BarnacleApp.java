...
public class BarnacleApp extends android.app.Application {
    ...
    final static String ACTION_CLIENTS = "net.rize.mesh.SHOW_CLIENTS";
    final static String ACTION_TOGGLE = "net.rize.mesh.TOGGLE_STATE";
    final static String ACTION_CHECK = "net.rize.mesh.CHECK_STATE";
    final static String ACTION_CHANGED = "net.rize.mesh.STATE_CHANGED";
        ...
        public void onReceive(Context context, Intent intent) {
            if (WifiManager.SCAN_RESULTS_AVAILABLE_ACTION.equals(intent.getAction())) {
                ...
                if (shouldDisableWifi) {
                	Log.e(TAG,"on Receive(), Disabling Wifi");
    ...
    public void onCreate() {
        ...
        notification = new Notification(R.drawable.rize, getString(R.string.notify_running), 0);
        ...
        notificationError = new Notification(R.drawable.rize_error,
    ...
    public void onTerminate() {
        ...
        if (shouldDisableWifi) {
        	Log.e(TAG,"onTerminate() shouldisableWifi");
            wifiManager.setWifiEnabled(false);
        }
        try {
    ...
    public int getState() {
        if (service != null){
        	Log.e(TAG,"getState()"+Integer.toString(service.getState()));
        	return service.getState();
            }
        Log.e(TAG,"getState() StateStopped");
    ...
    static void broadcastState(Context ctx, int state) {
        ...
        ctx.sendBroadcast(intent, "net.rize.mesh.ACCESS_STATE");
    }
    ...
    void failed(int err) {
    	Log.e(TAG,"failed() "+Integer.toString(err));
        if (statusActivity != null) {
            if (err == ERROR_ROOT) {
    ...
    protected boolean prepareBinaries() {
      ...
      //  if (!installIfNeeded(newVersion, R.raw.nat,   "nat")) 	  return false;
        ...
        /*aggiunto per fare funzionare olsr*/
        if (!installIfNeeded(newVersion, R.raw.olsrd,  "olsrd"))    return false;
        if (!installIfNeeded(newVersion, R.raw.olsrd_conf,  "olsrd_conf"))    return false;
        if (!installIfNeeded(newVersion, R.raw.wifi,  "wifi"))    return false;
        if (!installIfNeeded(newVersion, R.raw.kill_olsr,  "kill_olsr"))    return false;
        if (!installIfNeeded(newVersion, R.raw.shutdown_olsr,  "shutdown_olsr"))    return false;
        if (!installIfNeeded(newVersion, R.raw.httpinfo,  "httpinfo"))    return false;
        if (!installIfNeeded(newVersion, R.raw.txtinfo,  "txtinfo"))    return false;
        if (!installIfNeeded(newVersion, R.raw.get_info,  "get_info"))    return false;
        if (!installIfNeeded(newVersion, R.raw.nat_sleep,  "nat_sleep"))    return false;
        if (!installIfNeeded(newVersion, R.raw.start_dhcp,  "start_dhcp"))    return false;
        if (!installIfNeeded(newVersion, R.raw.dhcp_avvio,  "dhcp_avvio"))    return false;
...
//        if (!installIfNeeded(newVersion, R.raw.dnsmasq, "dnsmasq")) return false;
   ...
   /*  if (service != null) {
    ...
    void cleanUpNotifications() {
    	Log.e(TAG,"cleanUpNotifications()");
