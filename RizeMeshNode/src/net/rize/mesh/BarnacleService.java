...
public class BarnacleService extends android.app.Service {
	...
	private Process processNat = null ; //the nat process
	private Process processDhcp=null;//the dhcp process
	private LocalSocket nat_ctrl = null;
	// output monitoring threads
	private Thread[] threads = new Thread[6];//2 in the oldest
	...
	boolean natSpetta=false; //Serve a controllare l'avvio di nat
	...
	public void statsRequest(long delay) {
		Log.e(TAG,"statsRequest()");
		Message msg = mHandler.obtainMessage(MSG_STATS);
		mHandler.sendMessageDelayed(msg, delay);
	}

	public int getState() {
		Log.e(TAG,"getState() ="+Integer.toString(state));
		return state;
	}

	public boolean hasFiltering() {
		Log.e(TAG,"hasFiltering()");
	...
	private void handle(Message msg) {
		...
		case MSG_NETSCHANGE:
			...
			if (wifiState == WifiManager.WIFI_STATE_DISABLED) {
				...
				if(app.prefs.getBoolean("wan_start", false) && checkUplink()==false)
				{//Insert to fix checkupLink
					Toast.makeText(getApplicationContext(), "Umts interfaced is not connected!", Toast.LENGTH_LONG).show();
					...
					if (app.prefs.getBoolean("wan_start",false) && !app.findIfWan()) {//se la wan è down non vado in findIfWan
	...
	private boolean startProcess() {
		...
		//Adding Nat activation
		if(app.prefs.getBoolean("wan_start", false))
		{
			final Timer spetta=new Timer();
			final Handler handler=new Handler();

			spetta.schedule(new TimerTask(){
				@Override
				public void run() {
					...
					handler.post(new Runnable() {						
						...
						public void run() {
							// TODO Auto-generated method stub
							try{
								ProcessBuilder pbNat = new ProcessBuilder();
								pbNat.command("./nat_sleep").directory(getFilesDir());
								processNat =pbNat.start();								

							}catch(Exception e){
								log(true, String.format(getString(R.string.execerr), "./nat"));
								Log.e(TAG, "Failed to start Nat", e.fillInStackTrace());
							}
						}
					});
				}            	
		...
		//Adding Dhcp activation
		if(app.prefs.getBoolean(getString(R.string.dhcp_start), false))
		{
			final Timer spetta=new Timer();
			final Handler handler=new Handler();
				...
				public void run() {
					...
					handler.post(new Runnable() {						
						...
						public void run() {
							// TODO Auto-generated method stub
							try{
								ProcessBuilder pbDhcp = new ProcessBuilder();
								pbDhcp.command("./start_dhcp").directory(getFilesDir());
								processDhcp =pbDhcp.start();								

							}catch(Exception e){
								log(true, String.format(getString(R.string.execerr), "./dhcp"));
								Log.e(TAG, "Failed to start Dhcp", e.fillInStackTrace());
							}
						}
					});
				}            	
		...
		Log.e(TAG,"startProcess restituisco true");
		return true;
	}
	...
	private void stopProcess() {
		...
		try {
			...
			pbKill.command("./shutdown_olsr").directory(getFilesDir());
			// TODO: consider putting brncl.ini in pb.environment() instead of using ./setup
			kill = pbKill.start(); //Runtime.getRuntime().exec(cmd);

		} catch (Exception e) {
			log(true, String.format(getString(R.string.execerr), "Olsrd Kill"));
